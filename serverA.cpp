...
int main(){
     // Load the data file for server A
    loadFile("dataA.txt");

    // Create UDP socket for server A from Beej's Guide
    ...
    cout << "Server A is up and running using UDP on port " << PORT_A << "\n";
    ...
    for(;;) {
        ...
        if(receivedMessage=="LIST?"){
            ...
            cout << "Server A has sent a player list to Main Server\n";
        ...
        if(receivedMessage.rfind("QUERY,", 0) == 0) {
            ...
            if(queryTokens.size() >= 3) {
                ...
                cout << "Server A has received a request for player " << queriedPlayerName
                ...
                if((isplayer1Name && expectedTurn != 0) || (isplayer2Name && expectedTurn != 1) ){
                    ...
                    cout << "Server A found 0 available slots in column " << columnIndex << ".\n";
                    sendto(sock, err.c_str(), err.size(), 0, (sockaddr*)&senderAddress, senderAddressLen);
                    cout << "Server A has sent the results to Main Server\n";
                    continue;
                }
                ...
                cout << "Server A found " << gameState.availableSlotsPerColumn[selectedColumnIndex] 
