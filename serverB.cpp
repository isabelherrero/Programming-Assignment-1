...
// Struct to store the current game state for Server B from previous project
struct GameState {
    // Name of player 1
    string player1;
    // Name of player 2
...
// Function to load game data from dataB.txt
void loadDataFile() {
    // Open dataB.txt for reading
    ifstream dataFile("dataB.txt");
...
// Function to save current game data back to dataB.txt from previous project
void saveDataFile() {
    // Open dataB.txt for writing
    ofstream dataFile("dataB.txt");
...
// Main function: entry point of Server B from Beej's Guide and previous project
int main() {
    ...
    // Bind to Server B's UDP port
    servaddr.sin_port = htons(SERVERB_UDP_PORT);
    ...
    // Print that Server B is running and its port
    printf("The Server B is up and running using UDP on port %d.\n", SERVERB_UDP_PORT);
    
    // Load game data
    // Read initial game state from dataB.txt
    loadDataFile();
    ...
    while(1) {
        ...
        if(clientRequestMessage == "PLAYER_LIST") {
            ...
            // Log action
            printf("Server B has sent the player list to Main Server.\n");
        }
        // If request is a VIEW request
        else if(clientRequestMessage.substr(0, 4) == "VIEW") {
            // Log receipt
            printf("Server B has received a view request to send current board state.\n");
            ...
            sendto(sockfd, serverReplyMessage.c_str(), serverReplyMessage.length(), 0, 
                   (const struct sockaddr *)&client_addr, addressLength);
            // Log action
            printf("Server B sends the current board state to Main Server.\n");
        }
        // If request is a MOVE request
            ...
            // Log the move
            printf("Server B has received a move request for column %d.\n", boardColumn);
            ...
            if(moveResultCode == -1) {
                ...
                // Log invalid move
                printf("Move to column %d is invalid in Server B.\n", boardColumn);
            // Move was accepted
            } else {
                ...
                // Log success
                printf("Server B accepted move in column %d. Board state updated.\n", boardColumn);
            }
            ...
            printf("Server B has sent move result and updated board to Main Server.\n");
        }
    }
