...
int main(){
     // Load the data file for Server C
    loadFile("dataC.txt");

    // Create UDP socket for Server C from Beej's Guide
    ...
    cout << "Server A is up and running using UDP on port " << PORT_C << "\n";
    ...
    for(;;) {
        ...
        if(receivedMessage=="LIST?"){
            ...
            cout << "Server C has sent a player list to Main Server\n";
        ...
        if(receivedMessage.rfind("QUERY,", 0) == 0) {
            ...
            if(queryTokens.size() >= 3) {
                ...
                cout << "Server C has received a request for player " << queriedPlayerName
                ...
                if((isplayer1Name && expectedTurn != 0) || (isplayer2Name && expectedTurn != 1) ){
                    ...
                    cout << "Server C found 0 available slots in column " << columnIndex << ".\n";
                    sendto(sock, err.c_str(), err.size(), 0, (sockaddr*)&senderAddress, senderAddressLen);
                    cout << "Server C has sent the results to Main Server\n";
                    continue;
                }
                ...
                cout << "Server C found " << gameState.availableSlotsPerColumn[selectedColumnIndex] 
